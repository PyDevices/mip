...
"""SDL-style key codes, names, and modifier masks.

Import as ``import keys`` and use ``keys.K_a``, ``keys.KMOD_CTRL``,
``keys.keyname(code)``, ``keys.chord_matches(...)``.
"""
...
def keyname(x):
    ...
    return _keytable.get(x, "Unknown")
...
def key(x):
    ...
    Args:
        x: A name from :func:`keyname` (e.g. ``"Return"``).

    Returns:
        int: Matching ``K_*`` code.

    Raises:
        ValueError: When ``x`` is not a known key name.
    """
    return list(_keytable.keys())[list(_keytable.values()).index(x)]


def modname(x):
    """Return the human-readable name for modifier mask ``x``."""
    return _modtable.get(x, "Unknown")
...
def mod(x):
    ...
    Args:
        x: A name from :func:`modname` (e.g. ``"Ctrl"``).

    Returns:
        int: Matching ``KMOD_*`` mask.

    Raises:
        ValueError: When ``x`` is not a known modifier name.
    """
    return list(_modtable.keys())[list(_modtable.values()).index(x)]
...
_MOD_GROUPS = (KMOD_CTRL, KMOD_SHIFT, KMOD_ALT, KMOD_GUI)
